...
<body>
    <div class="donate">
        <?php
        $campain_name = $_GET['campain_name'];
        $description = $_GET['description'];
        $goal = $_GET['goal'];
        $organization_name = $_GET['organization_name'];
        $pic_url = 'http://localhost/wordpress/wp-content/plugins/DonorHub/inc/upload/';
        $campain_img = $pic_url . $_GET['campain_img'];
        $output = '<img src="' . $campain_img . '" alt="Photo is not Here" " width=30% height=30%>';
        $output .= "<h3>Campain Name: {$campain_name}</h3>";
        $output .= "<center><div id='desc'><h3>Description: {$description}</h3></div></center>";
        $output .= "<h3>Goal: {$goal}</h3>";
        if (!empty($organization_name)) {
            $output .= "<h3>organization_name: {$organization_name}</h3>";
        }
        echo $output;
        ?>
        <button>Donate</button>
        <form name="donAmount" id="donAmount">
            <input type="radio" value="5" name="amount" id="three" />
    ...
    <section id="confirm">
        <h2>Thank You!</h2>
        <p>
            Your <span class="amount">₹25</span> donation has been donated to
            <?php echo $campain_name; ?> Campaign
        </p>
    </section>
